...
import { createHydrationLog, updateHydrationLog } from "./graphql/mutations";
...
const HydrationLog = ({ user }) => {
  ...
  // Fetch hydration logs for the current day
  const fetchHydrationLogs = useCallback(async () => {
    try {
      ...
      // Calculate total hydration for the day
      const total = logs.reduce(
        (sum, log) => sum + parseInt(log.amount, 10),
        0
      );
      setDailyTotal(total);
    } catch (error) {
